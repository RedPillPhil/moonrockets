...
async function loadSQLJS() {
    ...
    initSqlJs(dbconfig).then(function(SQL){
      ...
      xhr.open('GET', '/assets/database3.db', true);
...
function sortRR() {
  galleryresult = [];
  db.each(
    "SELECT id FROM lilmoonrockets ORDER BY rrvalue ASC",
    [],
    function (row){
      galleryresult.push(row.id);
    },
    function() {
...
function compileDBStatement() {
  ...
  galleryresult = [];
  let query = squel.select().from("lilmoonrockets").field("id").order("rrvalue");
  ...
  .promise()
    .done( function() {
        //console.log("done");
        var ismintedcheck = $("#isminted_select").val();

        //console.log("ismintedcheck", ismintedcheck);
        // let res = db.exec();
        ...
        db.each(
          ...
          [],
          function (row){
            //if($("#isminted_select").val() == "0");
            if(ismintedcheck === "0") {
              // only show those not yet minted.
              if(!isMinted(getTokenFromArtwork(row.id))) {
                galleryresult.push(row.id);
              }
            } else if(ismintedcheck === "1") {
              if(isMinted(getTokenFromArtwork(row.id))) {
                galleryresult.push(row.id);
              }
            } else {
...
const getLMR = function () {
  if(currentpage === "gallery-new" || currentpage === "wallet") {
    $('#modalNFT').modal('hide');
  }
  ...
  ethereum
    ...
    .catch((err) => {
      if (err.code === 4001) {
        ...
        showToast(`Please connect to MetaMask to use LiL Moon Rockets!`, "alert");
      } else {
...
async function showSuccessDialog() {
  ...
  if(currentAccount != null) {
    ...
    $('.user_lmr_supply').text(amountrockets + " LiL Moon Rocket" + (amountrockets == 1 ? "": "s"));
...
function isMinted(token_number) {
  // returning whether this token is minted or not.
  if(contractsInitialized) {
    if(parseInt(token_number) < parseInt(nftsupply)) {
      return true;
    } else {
      return false;
    }
...
async function modalNFT_rename(){
  ...
  $.dialog.prompt(`Give NFT #${tokenId} a new name.`, 'Min: 1 char, max: 25 chars.\nOnly Alpha Numeric and spaces are valid characters.<br /><snmall>You will need to have 1337 NYR tokens for a name change, so claim those first</small>', input, async function(newname){
    ...
    $.dialog.confirm('Confirm new name?',`Naming a LiL Moon Rocket costs 1337 NYR (and some gas).<br /><b>Confirm new name of NFT #${tokenId}: <code>${newname}</code>?</b>`, async function(res){
      if(!res) {
...
function getTweetLink(tokenId, nftname) {
  ...
  let totweet = "Imma go to the moon with LiL Moon Rockets";

  if(nftname) {
    totweet = `Look at my LiL Moon Rocket ${nftname}`;
  }
...
async function loadNFTModal(tokenId) {
  ...
  $('#traitname_isminted').text((await isMinted(tokenId) ? "Yes": "Not yet!"));

  $('#modalNFT_gallerydeeplink').attr('href', `https://lilmoonrockets.com/gallery.html?token=${tokenId}`);
  //$('#modalNFT_gallerydeeplink').text(`Deeplink #${tokenId}`);
  // functions:
...
async function fillNFTStats(tokenId) {
  ...
  $('#traitname_rrrank').text(dbres.rrranking);

  // get rrrating
  for(var i = 0; i < rrratings.length; i++) {
    if(dbres.rrranking <= rrratings[i].until) {
      $('#traitname_rrrating').text(`Top ${rrratings[i].category}%`);
      break;
    } else {
      $('#traitname_rrrating').text(`-`);
    }
  }
